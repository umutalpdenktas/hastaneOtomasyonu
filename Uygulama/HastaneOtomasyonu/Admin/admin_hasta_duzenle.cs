...
{
    public partial class admin_hasta_duzenle : Form
    {
        public admin_hasta_duzenle()
        {
            InitializeComponent();
        }
        ...
        private void admin_hasta_duzenle_Load(object sender, EventArgs e)
        {
            //HASTANIN KODUNA GORE BİLGİERLİNİ DOLDURMA
            baglanti.Open();
            komut = new SqlCommand("SELECT * FROM hasta WHERE hasta_kodu =@veri ", baglanti);
            komut.Parameters.AddWithValue("@veri", admin_hastalistesi.gonderilecekveri);
